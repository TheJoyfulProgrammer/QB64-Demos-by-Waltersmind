...
DO
    ...
    FOR i = 0 TO UBOUND(ufo)
        ...
        _PUTIMAGE (WindowWidth - (UFO(i).x - (UFOwidth / 2)) - UFOwidth, WindowHeight - (UFO(i).y - (UFOheight / 2)) - UFOheight), UFOimage, 0, (UFO(i).XOffset, UFO(i).YOffset)-(UFO(i).XOffset + UFOwidth - 1, UFO(i).YOffset + UFOheight - 1)
    NEXT
