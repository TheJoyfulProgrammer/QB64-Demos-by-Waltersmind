...
DIM SHARED ClockSize2 AS _FLOAT
...
Pendulum& = _NEWIMAGE(100, 300, 32)

degree = 3.141592654 / 180

SCREEN PrimaryScreenSaved&
...
FOR ShadowWidth = 1 TO 0.70 STEP -0.05
    drawImage ClockShadow&, PrimaryScreenSaved&, 400, 590, _WIDTH(ClockShadow&) / 2, _HEIGHT(ClockShadow&) - 20, 0, ShadowWidth
NEXT
...
DO
    ...
    _PUTIMAGE (0, 0)-(_WIDTH(PrimaryScreenBackBuffer&), _HEIGHT(PrimaryScreenBackBuffer&)), PrimaryScreenSaved&, PrimaryScreenBackBuffer&

    radius# = 15
    speed# = 3
    angle! = (radius# * SIN(TinyTimer# * speed#)) * degree
    drawImage Pendulum&, PrimaryScreenBackBuffer&, 399, 214, _WIDTH(Pendulum&) / 2, -79, angle!, 1

    x1 = 199: y1 = 25: x2 = x1 + 400: y2 = y1 + 400
    ClockSize2 = .35 - (9 * SIN(TinyTimer# * 3)) * degree

    drawImage ClockScratch1&, PrimaryScreenBackBuffer&, 399, 214, 199, 199, 0, 1 + (15 * SIN(TinyTimer# * 3)) * degree
    drawImage ClockScratch1&, PrimaryScreenBackBuffer&, 100, 100, 199, 199, 0, ClockSize2
    drawImage ClockScratch1&, PrimaryScreenBackBuffer&, 700, 100, 199, 199, 0, ClockSize2
    drawImage ClockScratch1&, PrimaryScreenBackBuffer&, 100, 500, 199, 199, 0, ClockSize2
    drawImage ClockScratch1&, PrimaryScreenBackBuffer&, 700, 500, 199, 199, 0, ClockSize2
